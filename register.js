...
async function performCheckin(wallet) {
  try {
    ...
    if (response.data.errors) {
      ...
      console.error(kleur.red(`Checkin Task failed for ${wallet.address}: ${errorMessage}`));
      return {
        success: false,
        message: errorMessage,
      };
    }
    console.log(kleur.green(`Checkin Task successful for ${wallet.address}`));
    return {
      success: true,
      data: response.data,
    };
    ...
    console.error(
      kleur.yellow(`Checkin Task failed for ${wallet.address}: `),
      error.message
    );
    throw error;
  }
...
async function followTask(wallet) {
  try {
    ...
    if (response.data.errors) {
      const errorMessage = response.data.errors[0]?.message || "Unknown error";
      console.error(kleur.red(`Follow Task failed for ${wallet.address}: ${errorMessage}`));
      return {
        success: false,
        message: errorMessage,
      };
